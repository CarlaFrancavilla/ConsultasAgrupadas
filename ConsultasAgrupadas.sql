...
inscribieron en ese día? -ESTE ME COSTÓ UN POQUITO JAJA
SELECT fecha, SUM(cantidad) AS inscritos_por_dia
FROM INSCRITOS
GROUP BY fecha
...
solo el primero)
SELECT fecha, SUM(cantidad) AS inscritos_blog
FROM INSCRITOS
WHERE fuente = 'Blog'
GROUP BY fecha
ORDER by inscritos_blog DESC
LIMIT 1;
